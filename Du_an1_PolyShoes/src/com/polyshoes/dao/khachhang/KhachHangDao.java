...
//    }
    ...
    public List<KhachHang> selectHoaDon1(String MaKH) {
        String sql = "SELECT Khach_Hang.MaKH,Hoa_Don.MaHD, Khach_Hang.HoTen, Khach_Hang.SDT, Hoa_Don.TongTien, Hoa_Don.NgayNhan, Hoa_Don.TrangThai, CONCAT(DiaChi.Xa ,',' ,DiaChi.Quan,',' ,DiaChi.Tinh) AS DiaChiChiTiet\n"
                + "FROM     Khach_Hang INNER JOIN\n"
                + "                  Hoa_Don ON Khach_Hang.ID = Hoa_Don.ID INNER JOIN\n"
                + "                  DiaChi ON Khach_Hang.ID = DiaChi.ID  where MaKH like ?";
